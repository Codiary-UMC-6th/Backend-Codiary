...
public class PostConverter {
    ...
    public static PostResponseDTO.PostWithBookmarkDTO toPostWithBookmarkDTO(Post post, boolean isBookmarked, int bookmarkCount) {
        List<String> postCategories = post.getCategoriesList().stream()
                .map(Category::getName)
                .collect(Collectors.toList());

        return PostResponseDTO.PostWithBookmarkDTO.builder()
                .postId(post.getPostId())
                .memberId(post.getMember().getMemberId())
                .authorNickname(post.getMember().getNickname())
                .teamId(post.getTeam() != null ? post.getTeam().getTeamId() : null)
