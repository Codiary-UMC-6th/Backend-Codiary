...
/*
    ...
    @GetMapping("/posts")
    public ApiResponse<PostResponseDTO.MemberPostPreviewListDTO> getMyDiaries(@RequestParam(value = "memberId", required = true) Long memberId, @RequestParam(value = "projectId", required = false) Long projectId, @PageableDefault(size=6) Pageable pageable){
        Page<Post> posts = memberQueryService.getMyPosts(memberId, projectId, pageable);
